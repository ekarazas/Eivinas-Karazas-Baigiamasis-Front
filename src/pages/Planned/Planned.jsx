...
import * as S from "./Planned.style";
...
const Planned = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8080/v1/todos/planned", {
      headers: {
  ...
  return (
    <>
      ...
      <Wrapper>
        <S.Planned>
          <S.PageTitle>Planned Tasks</S.PageTitle>
          {isLoading ? (
            ...
            <S.StyledList>
              {todos.length !== 0 ? (
                todos
                  .sort((a, b) => {
                    let c = new Date(a.due_date);
                    let d = new Date(b.due_date);
                    return d - c;
                  })
                  .map((todo) => (
                    <Todo
